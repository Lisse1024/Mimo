...
OPERATIONAL_METADATA_RE = re.compile(
    ...
    r"platform connector|media processing|素材处理|平台线索|平台[:：].*浏览器|抖音网页版|Microsoft|微软浏览器|"
    r"浏览器浏览器|当前窗口|窗口标题|窗口进程|视频时长|进度条显示|进度条|00:\d{2}/00:\d{2}|frame-\d+\.jpg|sampling:|duration:|"
...
NON_GROWTH_SCRIPT_MATERIAL_RE = re.compile(
    r"作者声明|免责声明|虚构演绎|仅供娱乐|剧情演绎|请勿对号入座|无不良引导|"
    r"素材来源|版权归原作者|侵删|如有侵权|平台声明|风险提示|"
    r"未成年人请勿模仿|危险动作请勿模仿",
    re.IGNORECASE,
)

UNSUPPORTED_SCRIPT_INFERENCE_RE = re.compile(
    r"热门\s*CP|CP\s*向|BL\s*同人|同人受众|吸引[^，。；\n]{0,20}受众|"
    r"粉丝一定|必然吸引|天然适合|已经验证[^，。；\n]{0,20}人群|短视频平台常见|"
    r"符合原曲情绪|引发[^，。；\n]{0,20}共鸣|情绪共鸣|季节共鸣|孤独感氛围|秋日/深夜|目标受众|用户画像|夜间寻求",
    re.IGNORECASE,
)

VISIBLE_METRIC_SCRIPT_MATERIAL_RE = re.compile(
    r"互动数据|右下角互动|点赞\s*\d+|评论\s*\d+|收藏\s*\d+|分享\s*\d+|播放量|浏览量|获赞|粉丝数|转发\s*\d+|"
    r"视频时长|进度条显示|进度条|00:\d{2}/00:\d{2}",
    re.IGNORECASE,
)

EXTRA_OPERATIONAL_METADATA_RE = re.compile(
    r"用户请求|平台线索|素材处理|桌面助手|助手弹窗|弹窗出现|弹窗关闭|当前窗口|窗口标题|窗口进程|视频时长|进度条显示|进度条|抖音网页版|微软浏览器|浏览器浏览器",
    re.IGNORECASE,
)

EXTRA_UNSUPPORTED_SCRIPT_INFERENCE_RE = re.compile(
    r"热门\s*CP|CP\s*向|同人受众|吸引[^，。；\n]{0,20}受众|符合原曲情绪|短视频平台常见|流量密码|治愈陪伴感|营造[^，。；\n]{0,20}感|目标受众|用户画像|夜间寻求|情绪共鸣|季节共鸣|孤独感氛围|引发[^，。；\n]{0,20}共鸣",
    re.IGNORECASE,
)

EXTRA_PAGE_CONTEXT_MATERIAL_RE = re.compile(
    r"搜索框|搜索栏|搜索词|账号信息|作者信息|发布账号|合集标签|集数标题|章节信息|平台UI|抖音原生界面|按钮布局|点赞按钮|评论按钮|收藏按钮|分享按钮|桌面助手|助手弹窗|弹窗出现|弹窗关闭|发布时间|时间戳|抖音精选|导航栏|推荐页|关注页",
    re.IGNORECASE,
)

EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE = re.compile(
    r"UGC|OGC|éŸ³ä¹åšä¸»ç‰¹å¾|ç›´æŽ¥ç»™éŸ³ä¹ä»·å€¼|Cold Open|é›¶å¸§èµ·æ‰‹|å¤§å®¶å¥½æˆ‘æ˜¯|visual\s*/\s*frame",
    re.IGNORECASE,
)

EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE_2 = re.compile(
    r"UGC|OGC|Cold Open|visual\s*/\s*frame|source_type|frame:\s*\d{1,2}:\d{2}|"
    r"\u5927\u5bb6\u597d\u6211\u662f|XX|\u5e9f\u8bdd|\u96f6\u5e27\u8d77\u624b|\u76f4\u63a5\u7ed9\u97f3\u4e50\u4ef7\u503c|"
    r"\u97f3\u4e50\u535a\u4e3b\u7279\u5f81|\u539f\u751f\u62cd\u6444\u8d28\u611f|\u65e0\u540e\u671f\u7279\u6548",
    re.IGNORECASE,
)

PAGE_CONTEXT_MATERIAL_RE = re.compile(
    r"搜索框|搜索栏|搜索词|搜索页面|搜索结果|账号信息|作者信息|发布账号|@\S+|"
    r"集数标题|章节信息|第\d+章[:：]|合集标签|合集：|合集[:：]|第\d+集[:：].*#|发布时间|发布于|主页|关注按钮|"
    r"点赞按钮|评论按钮|收藏按钮|分享按钮|转发按钮|按钮布局|原生界面元素|抖音原生界面|评论区入口|平台UI|导航栏|抖音精选|推荐页|关注页|时间戳|\d+\s*小时前|"
    r"更新晚了|抱歉最近更新|最近更新晚|平台[:：].*浏览器|抖音网页版|Microsoft|微软浏览器|浏览器浏览器",
    re.IGNORECASE,
)

CORE_HOOK_AUXILIARY_RE = re.compile(
    r"^(标题|标签|搜索框|搜索栏|搜索词|账号信息|作者信息|合集标签|集数标题|章节信息|平台UI|平台|时间戳|视频时长|进度条)[:：]|"
    r"标题直接|标题引用|抱歉最近更新|最近更新晚|\d+\s*小时前|抖音精选|导航栏|抖音网页版|Microsoft|微软浏览器|浏览器浏览器",
    re.IGNORECASE,
)

SCRIPT_GROWTH_INTERPRETATION_RE = re.compile(
    r"(?:，|；|;)?(?:建立与[^，。；\n]{0,30}情绪连接|提高[^，。；\n]{0,20}|吸引[^，。；\n]{0,20}|"
    r"验证[^，。；\n]{0,30}|带来[^，。；\n]{0,20}|触发[^，。；\n]{0,20}|形成[^，。；\n]{0,20}人群|"
    r"\d{2}后[^，。；\n]{0,20}记忆|集体记忆|情怀金曲|氛围感演奏|配合[^，。；\n]{0,20}关键词|"
    r"引发[^，。；\n]{0,20}共鸣|情绪共鸣|季节共鸣|孤独感氛围|符合原曲情绪|秋日/深夜[^，。；\n]{0,20}|目标受众[^，。；\n]{0,30}|用户画像[^，。；\n]{0,30})",
    re.IGNORECASE,
)
...
def _compact_visible_phrase(value: Any, max_len: int = 48) -> str:
    ...
    text = re.sub(r"\s*[ï¼ˆ(]\s*visual\s*/\s*frame\s*[:ï¼š][^)ï¼‰]*[)ï¼‰]", "", text, flags=re.IGNORECASE)
    text = re.sub(r"\s*[ï¼ˆ(]\s*frame\s*[:ï¼š][^)ï¼‰]*[)ï¼‰]", "", text, flags=re.IGNORECASE)
    if OPERATIONAL_METADATA_RE.search(text) or EXTRA_OPERATIONAL_METADATA_RE.search(text):
        return ""
    return text[:max_len].strip()


def _is_non_growth_script_material(value: Any) -> bool:
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    return bool(text and NON_GROWTH_SCRIPT_MATERIAL_RE.search(text))


def _script_material_allowed(value: Any) -> bool:
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    return bool(text) and not _is_non_growth_script_material(text) and not PAGE_CONTEXT_MATERIAL_RE.search(text) and not EXTRA_PAGE_CONTEXT_MATERIAL_RE.search(text) and not VISIBLE_METRIC_SCRIPT_MATERIAL_RE.search(text)


def _is_page_context_material(value: Any) -> bool:
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    return bool(text and (PAGE_CONTEXT_MATERIAL_RE.search(text) or EXTRA_PAGE_CONTEXT_MATERIAL_RE.search(text)))


def _is_auxiliary_core_hook(value: Any) -> bool:
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    return bool(text and (CORE_HOOK_AUXILIARY_RE.search(text) or PAGE_CONTEXT_MATERIAL_RE.search(text) or EXTRA_PAGE_CONTEXT_MATERIAL_RE.search(text)))


def _direct_script_material(value: Any) -> str:
    text = _compact_visible_phrase(value, 140)
    text = re.sub(r"[ï¼Œ,ï¼›;]?\s*æ— ['â€œâ€â€˜â€™\"]?å¤§å®¶å¥½æˆ‘æ˜¯[^ï¼Œã€‚ï¼›\n]*", "", text)
    text = re.sub(r"[ï¼Œ,ï¼›;]?\s*ç›´æŽ¥ç»™éŸ³ä¹ä»·å€¼", "", text)
    if not _script_material_allowed(text):
        return ""
    if "字幕文本序列" in text or text.count("→") >= 3:
        parts = [part.strip(" '\"“”‘’：:") for part in re.split(r"→|->", text) if part.strip(" '\"“”‘’：:")]
        if parts:
            first = re.sub(r"^字幕文本序列[:：]?", "", parts[0]).strip(" '\"“”‘’：:")
            compact_parts = [first, *parts[1:3]]
            compact_parts = [part for part in compact_parts if part][:3]
            return f"字幕递进：{' → '.join(compact_parts)}"[:120]
    quote = re.search(r"[\"'“”‘’「](.{2,36}?)[\"'“”‘’」]", text)
    if quote and re.search(r"情绪共鸣钩子|台词金句|核心哲理|核心看点", text):
        return f"台词：{quote.group(1).strip()}"
    hashtags = re.findall(r"#[\w\u4e00-\u9fff-]+", text)
    if hashtags and (UNSUPPORTED_SCRIPT_INFERENCE_RE.search(text) or EXTRA_UNSUPPORTED_SCRIPT_INFERENCE_RE.search(text)):
        cleaned_tags = []
        for tag in hashtags:
            tag = re.split(r"这是|吸引|热门|CP\s*向|BL|同人|受众|粉丝|必然|天然|已经验证", tag, maxsplit=1, flags=re.IGNORECASE)[0]
            if len(tag) > 1:
                cleaned_tags.append(tag)
        return f"标签：{' '.join(dict.fromkeys(cleaned_tags))}" if cleaned_tags else ""
    subtitle = re.search(r"(字幕\s*\d{1,2}:\d{2}[^，。；\n]{0,80})", text)
    if subtitle and (UNSUPPORTED_SCRIPT_INFERENCE_RE.search(text) or EXTRA_UNSUPPORTED_SCRIPT_INFERENCE_RE.search(text)):
        return subtitle.group(1).strip()
    if UNSUPPORTED_SCRIPT_INFERENCE_RE.search(text) or EXTRA_UNSUPPORTED_SCRIPT_INFERENCE_RE.search(text) or EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE.search(text) or EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE_2.search(text):
        return ""
    text = SCRIPT_GROWTH_INTERPRETATION_RE.sub("", text).strip(" ，。；;")
    text = EXTRA_UNSUPPORTED_SCRIPT_INFERENCE_RE.sub("", text).strip(" ，。；;")
    text = EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE.sub("", text).strip(" ï¼Œã€‚ï¼›;")
    text = EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE_2.sub("", text).strip(" ，。；;")
    return text


def _clean_script_material_list(items: list[str]) -> list[str]:
    cleaned: list[str] = []
    seen: set[str] = set()
    for item in items:
        material = _direct_script_material(item)
...
def _as_text_list(value: Any, limit: int = 12) -> list[str]:
    ...
    for item in value:
        ...
        text = _compact_visible_phrase(raw, 120)
...
def _ledger_materials(ledger: dict[str, Any], link_titles: list[str], link_tags: list[str]) -> dict[str, Any]:
    visible = _clean_script_material_list(_as_text_list(ledger.get("visible_facts"), 12))
    audio = _clean_script_material_list(_as_text_list(ledger.get("audio_facts"), 8))
    text = _clean_script_material_list(_as_text_list(ledger.get("text_facts"), 12))
    for title in link_titles[:3]:
        clean_title = _compact_visible_phrase(title, 120)
        clean_title = _direct_script_material(clean_title)
        if clean_title:
            text.append(f"标题：{clean_title}")
    for tag in link_tags[:5]:
        clean_tag = _compact_visible_phrase(tag, 60)
        clean_tag = _direct_script_material(clean_tag)
        if clean_tag:
    ...
    for item in timeline[:8]:
        ...
        parts = _clean_script_material_list(parts)
        if parts:
    ...
    for item in hooks[:8]:
        if not isinstance(item, dict):
            continue
        hook = _direct_script_material(item.get("hook"))
        evidence = _direct_script_material(item.get("evidence"))
        confidence = str(item.get("confidence") or "low")
        if (
            hook
            and hook not in {"冲突", "反差", "情绪价值", "情绪点", "爆点"}
            and _script_material_allowed(evidence or hook)
        ):
    ...
    all_materials = [*visible, *audio, *timeline_lines, *[item["hook"] for item in hook_items], *text, *characters]
    all_materials = _clean_script_material_list(all_materials)
    return {
...
def _single_work_content_type(asset_analysis: dict[str, Any], ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    performance_hits = [
        ...
        if any(keyword.lower() in item.lower() for keyword in ["演奏", "翻唱", "翻弹", "指弹", "弹唱", "吉他", "乐器", "伴奏", "曲谱", "歌曲", "歌词", "歌名", "cover"])
    ]
    true_media_hits = [
        item
        for item in evidence_pool
        if any(keyword in item for keyword in ["影视剪辑", "剧集片段", "电影片段", "综艺片段", "小品片段", "短剧片段", "人物对话", "剧情片段"])
    ]
    if performance_hits and (content_type in {"", "unknown", "media_clip"} or not true_media_hits):
        return {"content_type": "performance", "confidence": "medium", "evidence": performance_hits[:6]}
    if content_type:
        return {
    ...
    checks = [
        ...
        ("performance", ["演奏", "翻唱", "翻弹", "指弹", "弹唱", "吉他", "乐器", "表演", "伴奏", "曲谱", "歌曲", "歌词", "歌名", "cover"]),
    ...
    return {
        "content_type": "platform_native" if ledger_text else "unknown",
        "confidence": "low" if not ledger_text else "medium",
        "evidence": evidence_pool[:6],
    }


def _is_media_clip_type(content_type: str, source_type: str, ledger: dict[str, Any]) -> bool:
    if content_type in {"platform_native", "tutorial", "performance", "gameplay", "vlog", "product_review", "knowledge"}:
        return False
    text = json.dumps(ledger, ensure_ascii=False)
    if re.search(r"非影视|不是影视|非剧集|不是剧集|原创音乐|真人实拍吉他|吉他演奏", text):
        return False
...
def _discussion_material(materials: dict[str, Any], fallback: str) -> str:
    hooks = materials.get("hooks") if isinstance(materials.get("hooks"), list) else []
    for item in hooks:
        if not isinstance(item, dict):
            continue
        candidate = item.get("evidence") or item.get("hook")
        if _script_material_allowed(candidate):
            return str(candidate)
    all_items = materials.get("all") if isinstance(materials.get("all"), list) else []
    for item in all_items:
        if _script_material_allowed(item):
            return str(item)
    return fallback if _script_material_allowed(fallback) else INSUFFICIENT_CONTENT_EVIDENCE


def _question_from_material(material: str) -> str:
    text = _direct_script_material(material)
    raw = str(material or "").strip()
    if not text or EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE.search(raw) or EXTRA_SINGLE_WORK_SCRIPT_JUNK_RE_2.search(raw) or _is_page_context_material(raw):
        return "\u4f60\u66f4\u60f3\u770b\u5b8c\u6574\u5185\u5bb9\uff0c\u8fd8\u662f\u5e0c\u671b\u4e0b\u4e00\u6761\u8865\u66f4\u6e05\u695a\u7684\u6807\u9898/\u5b57\u5e55\u8bf4\u660e\uff1f"
    if re.search(r"å‰ä»–|æŒ‡å¼¹|æ¼”å¥|æ—‹å¾‹|æ­Œæ›²|æ›²å|æ‰‹éƒ¨|æŒ‰å¼¦|æ‹¨å¼¦", text):
        return "è¿™æ®µä½ æ›´æƒ³å¬å®Œæ•´æ¼”å¥ï¼Œè¿˜æ˜¯æƒ³çœ‹æŒ‡æ³•/æ›²åè¯´æ˜Žï¼Ÿ"
    if "→" in text:
        parts = [part.strip(" '\"“”‘’：:") for part in re.split(r"→", text) if part.strip(" '\"“”‘’：:")]
        if len(parts) >= 2:
            return f"你更想讨论「{parts[0]}」，还是后面「{parts[-1]}」这个转折？"
    quote = re.search(r"[\"'“”‘’「](.{2,36}?)[\"'“”‘’」]", text)
    focus = quote.group(1).strip() if quote else re.sub(r"^(台词|字幕|字幕递进)[:：]", "", text).strip()
    focus = focus[:36] if focus else "这句台词"
    return f"你觉得这段最值得讨论的是「{focus}」吗？"


def _clean_viewer_hook(value: Any, fallback: str) -> str:
    text = _direct_script_material(value)
    if text and not _is_auxiliary_core_hook(text):
        return text
    fallback_text = _direct_script_material(fallback)
    if fallback_text and not _is_auxiliary_core_hook(fallback_text):
        return fallback_text
    return INSUFFICIENT_CONTENT_EVIDENCE


def _time_label_from_material(material: str, fallback: str) -> str:
    match = re.search(r"(\d{1,2}:\d{2}(?:\s*[-~至]\s*\d{1,2}:\d{2})?)", material or "")
    if match:
        return f"{match.group(1).replace(' ', '')} 附近"
    return fallback
...
def _build_dynamic_script_steps(
    ...
    comment_focus = _discussion_material(materials, _material_at(materials, 3, source_name if source_name != "片源待确认" else first_material))
    source_evidence_for_script = [item for item in (source_info.get("evidence") or []) if _script_material_allowed(item)]
    low_note = "当前素材线索不足，只能做低置信最小测试。" if not materials["all"] else ""
    return [
        {
            "time": _time_label_from_material(first_material, "开头"),
        ...
        {
            "time": _time_label_from_material(second_material, "中段"),
            ...
            "evidence": "；".join(source_evidence_for_script or [second_material])[:180],
            "growth_reason": _growth_reason_for_step(1, second_material, content_type),
            "confidence": source_confidence,
        },
        {
            "time": _time_label_from_material(third_material, "中后段"),
        ...
        },
        {
            ...
            "caption_or_voiceover": _question_from_material(comment_focus),
...
def _adapt_single_work_experiment_fields(fields: dict[str, Any], content_type: str, is_media_clip: bool) -> dict[str, Any]:
    if is_media_clip or content_type == "media_clip":
        return fields
    if content_type == "performance":
        fields["growth_hypothesis"] = "如果下一条内容优先把可听声音、演奏动作、镜头氛围和曲名/字幕线索前置，完播率、分享率和评论点歌/求曲名关键词会比单纯复述标题更可诊断。"
        fields["test_action"] = "按 script_steps 做一条演奏/表演类小样本测试，发布后只观察完播、分享、评论点歌/求曲名和负反馈，不提前判断长期账号方向。"
        fields["decision_rules"] = [
            "3 秒留存低：把开头换成更清楚的声音进入、手部动作或曲名字幕。",
            "完播低：减少背景解释，提前进入最可识别的旋律/演奏段落。",
            "评论集中在点歌/求曲名/演奏细节：继续测试同类演奏结构，不直接宣布长期方向。",
            "负反馈集中在听不清/画面看不清/铺垫长：优化收音、光线和开头进入速度。",
        ]
    return fields
...
def _dynamic_single_work_hook(
    ...
    current_direct = _direct_script_material(current)
    if current and not any(marker in current for marker in generic_markers) and current_direct and not _is_auxiliary_core_hook(current_direct):
    ...
    ledger = work.get("fact_ledger") if isinstance(work.get("fact_ledger"), dict) else asset_analysis.get("fact_ledger")
    ledger_candidates = _ledger_materials(ledger, link_titles, link_tags).get("all") if isinstance(ledger, dict) else []
    ...
    phrase = next(
        (
            _direct_script_material(item)
            for item in flattened
            if _direct_script_material(item) and not _is_auxiliary_core_hook(_direct_script_material(item))
        ),
        "",
    )
    if phrase:
...
def build_rule_based_advisor_bundle(profile: dict[str, Any], asset_analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_current_video:
        ...
        viewer_hook = _clean_viewer_hook(viewer_hook, (script_steps[0].get("visual") if script_steps and isinstance(script_steps[0], dict) else ""))
        ...
        experiment_fields = _adapt_single_work_experiment_fields(experiment_fields, effective_content_type, is_media_clip)
    ...
    advisor_summary = {
        ...
        "one_sentence_diagnosis": "本轮主页诊断仅基于可见证据；缺少后台数据时，不声称长期方向、推荐状态或转粉效率已经被验证。",
        "core_judgements": [
            "具体栏目建议必须来自上游主页栏目方案，并且每条都要带证据依据。",
            "如果上游证据只能支持方向级判断，就做方向级小样本测试，不编具体标题。",
            "复盘使用 24/48 小时播放量、3 秒留存、完播率、评论关键词、主页点击率、关注转化和负反馈。",
        ],
        "evidence_chain": diagnosis + evidence if diagnosis or evidence else [asset_analysis.get("asset_summary", "基于主页截图、用户输入和可见线索做保守判断。")],
        "first_actions": [
            "先选择主页证据最明确的一个方向。",
            "连续发布 3 条同证据来源、同封面关键词、同字幕样式的内容。",
            "48 小时后回填统一指标，再决定是否生成更具体的栏目方案。",
        ],
        "first_content_task": {},
        "why_this_path": [
            "主页诊断与单条作品脚本分开处理；本轮只做栏目实验。",
            "避免关键词或垂类模板编造没有证据支撑的栏目标题。",
        ],
        "follow_up_question": "补充最近作品详情、封面标题截图、后台指标和评论区截图后，再生成更具体的证据驱动栏目方案。",
    ...
    tasks = normalize_tasks(
        [
            {"id": "advisor-task-1", "title": "补充主页证据", "goal": "补充最近 3 条作品详情、封面标题截图、后台指标和评论区截图。", "priority": "high", "status": "todo", "owner": "user", "source": "advisor_summary"},
            {"id": "advisor-task-2", "title": "选择证据最强方向", "goal": "从 homepage_evidence_map.content_patterns 里选择一个方向做小样本测试。", "priority": "high", "status": "todo", "owner": "advisor", "source": "advisor_summary"},
            {"id": "advisor-task-3", "title": "回填复盘指标", "goal": "记录 24/48 小时播放量、3 秒留存、完播率、评论关键词、主页点击率、关注转化和负反馈。", "priority": "medium", "status": "todo", "owner": "user", "source": "strategy"},
        ],
        profile,
    )
    return {"strategy": strategy, "advisor_summary": advisor_summary, "tasks": tasks}
